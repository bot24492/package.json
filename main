{
    "name": "Mirai_Project",
    "version": "1.2.15",
    "description": "Mirai Project được điều hành bởi KEY",
    "main": "mirai.js",
    "scripts": {
        "start": "node index",
        "login": "node login",
        "test": "node --trace-warnings --use_strict --async-stack-traces mirai"
    },
    "repository": {
        "type": "git",
        "url": "git+https://github.com/ThanhAli-Official/GbanMiraiProject.git"
    },
    "author": "KEY",
    "license": "GPL-3.0",
    "dependencies": {
        "@maihuybao/fca-unofficial": "^1.3.13-2",
        "@miraipr0ject/assets": "",
        "async": "^3.2.3",
        "axios": "^0.26.1",
        "better-sqlite3": "^7.5.1",
        "bluebird": "^3.7.2",
        "body-parser": "^1.20.0",
        "caesar-salad": "^2.1.0",
        "canvas": "^2.9.1",
        "chalk": "^4.1.2",
        "chalkercli": "^1.6.4",
        "cheerio": "^1.0.0-rc.10",
        "child_process": "",
        "cookie": "^0.5.0",
        "cookies": "^0.8.0",
        "crypto": "^1.0.1",
        "eval": "^0.1.8",
        "express": "^4.18.1",
        "extract-zip": "",
        "facebook-chat-api": "^1.8.0",
        "fast-speedtest-api": "^0.3.2",
        "fb-chat-api": "^10.4.1",
        "fca-can-get": "^1.3.15",
        "fca-disme": "^3.1.5",
        "fca-horizon-remake": "^4.7.3",
        "fca-unofficial": "^1.3.10",
        "fca-xuyen-get": "^1.3.15",
        "figlet": "^1.5.2",
        "fs": "0.0.1-security",
        "fs-extra": "^10.1.0",
        "gifencoder": "^2.0.1",
        "helyt": "^1.3.16",
        "index": "^0.4.0",
        "jimp": "^0.16.1",
        "json-stringify": "^1.0.0",
        "length": "0.0.1",
        "lyrics-finder": "^21.7.0",
        "minimist": "^1.2.6",
        "moment": "^2.29.2",
        "moment-timezone": "^0.5.34",
        "ms": "^2.1.3",
        "node-superfetch": "^0.2.3",
        "nomnom": "^1.8.1",
        "npmlog": "^6.0.1",
        "os": "^0.1.2",
        "parse-json": "^6.0.2",
        "pastebin-api": "^4.0.1",
        "path": "^0.12.7",
        "pidusage": "^3.0.0",
        "readline": "^1.3.0",
        "request": "^2.88.2",
        "semver": "^7.3.6",
        "sequelize": "^6.18.0",
        "sieudangyeu": "^1.3.16",
        "simple-youtube-api": "^5.2.1",
        "soundcloud-downloader": "^1.0.0",
        "sqlite": "^4.0.25",
        "sqlite3": "^5.0.2",
        "string-similarity": "^4.0.4",
        "systeminformation": "^5.11.9",
        "tinyurl": "^1.1.7",
        "totp-generator": "0.0.13",
        "wikijs": "^6.3.3",
        "youtube-search-api": "^1.1.0",
        "ytdl-core": "^4.11.0"
    }
}
